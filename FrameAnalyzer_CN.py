...
class Frame:
    ...
    def toDraw_3D(self,toplot,str_title = "色域空間"):
        ...
        fig.update_layout(title=str_title,
                          scene=dict(xaxis=dict(title='⇕彩度Saturation：颜色浓淡-和中心点xy平面上的距离\n近：淡❘远：浓', titlefont_color='black'),
                                     yaxis=dict(title='↺色相Hue：偏离y=0直线的角度\n右：红❘上：绿❘左：青❘下：紫', titlefont_color='black'),
                                     zaxis=dict(title='⤆明度Value：和中心点(0,0,0)的y轴垂直距离\n上：亮色❘下：暗色', titlefont_color='black'),
...
if __name__ == '__main__':
    ...
    img = imageio.v2.imread(file_name)
    var = img_TO_Colorwheel_parallel(img)
    var = Frame(var,True)
    print("Calculation Completed, now merging data.")
    var.toDraw_3D(var.toGetThreeDimArray(), str(file_name + " 的色相分析"))
